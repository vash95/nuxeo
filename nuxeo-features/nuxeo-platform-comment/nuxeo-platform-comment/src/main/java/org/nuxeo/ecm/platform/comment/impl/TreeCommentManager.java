...
public class TreeCommentManager extends AbstractCommentManager {
    ...
    public List<DocumentModel> getComments(CoreSession s, DocumentModel docModel) {
        // Check permissions
        checkReadCommentsPermissions(s, docModel.getRef());
        return CoreInstance.doPrivileged(s, session -> {
            PageProvider<DocumentModel> pageProvider = getCommentsPageProvide(session, docModel, null, null, true);
    ...
    public Comment getComment(CoreSession s, String commentId) {
        checkReadCommentsPermissions(s, new IdRef(commentId));
        return CoreInstance.doPrivileged(s, session -> {
    ...
    public PartialList<Comment> getComments(CoreSession s, String documentId, Long pageSize, Long currentPageIndex,
        ...
        // Check permissions
        checkReadCommentsPermissions(s, new IdRef(documentId));
        DocumentModel documentModel = s.getDocument(docRef);
        return CoreInstance.doPrivileged(s, session -> {
            PageProvider<DocumentModel> pageProvider = getCommentsPageProvide(session, documentModel, pageSize,
    ...
    public Comment getExternalComment(CoreSession session, String entityId) {
        ...
        checkReadCommentsPermissions(session, commentModel.getRef());
    ...
    public Comment createComment(CoreSession s, Comment comment) {
        checkReadCommentsPermissions(s, new IdRef(comment.getParentId()));
    ...
    public DocumentModel createComment(DocumentModel documentModel, DocumentModel commentDocModel) {
        ...
        checkCreateCommentPermissions(commentDocModel.getCoreSession(), documentModel);
    ...
    public void deleteComment(CoreSession s, String commentId) {
        DocumentModel commentDocumentModel = CoreInstance.doPrivileged(s, session -> {
            return getCommentDocumentModel(session, new IdRef(commentId));
        });
        removeComment(s, commentDocumentModel.getRef());
    }
    ...
    public String getLocationOfCommentCreation(CoreSession session, DocumentModel documentModel) {
        ...
        // reply on
        // a given comment) it will be under his comment
    ...
    @Override
    public DocumentRef getAncestorRef(CoreSession session, DocumentRef documentRef) {
    ...
    /**
     * Checks if the user related to the {@code session} can comments the given {@code docModel}.
     *
     ...
     */
    protected void checkCreateCommentPermissions(CoreSession s, DocumentModel docModel) {
        DocumentRef rootDocRef = CoreInstance.doPrivileged(s, session -> {
            return getRootDocument(session, docModel.getRef());
        });
     ...
     *         {@link CommentSecurityException}
     */
    protected void checkReadCommentsPermissions(CoreSession s, DocumentRef documentRef) {
        DocumentRef rootDocRef = CoreInstance.doPrivileged(s, session -> {
    ...
    protected void removeComment(CoreSession s, DocumentRef documentRef) {
        ...
        CoreInstance.doPrivileged(s, session -> {
            ...
            // FIXME ask Team: if we dont get any of `Comment` property, it will fail because schema `comment`
            // will be missing and at the time of notifyEvent (backward compatibility) it will try to loaded
            // and the doc is removed (end with exception DocumentNotFound)
     ...
     */
    public DocumentRef getRootDocument(CoreSession session, DocumentRef documentRef) {
    ...
    /**
     * We use the `Private` accessor here, to avoid any security issue. The check of the permissions is done at the
     * highest level
     *
     * @return the page provider that hold the comments documents model
     */
    @SuppressWarnings("unchecked")
    private PageProvider<DocumentModel> getCommentsPageProvide(CoreSession session, DocumentModel documentModel,
        ...
        // Depending on case, the `documentModel` can be a comment or not
        // if its a not comment, then we should retrieve all comments under `Comments` folder
        ...
        if (!documentModel.hasSchema(COMMENT_SCHEMA)) {
            DocumentModelList children = session.getChildren(documentModel.getRef(), COMMENTS_DIRECTORY_TYPE);
            if (!children.isEmpty()) {
                documentId = children.get(0).getId();
            }
        }
